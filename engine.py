...
def main() -> None:
    ...
    if _halt_state["fired"]:
        # Some models stop right after the tool call (or echo the call as
        # text) instead of continuing to Pass 2. Force the formatting pass.
        result = agent(
            "The audit is finished and the conflict above is resolved by the "
            "student's answer in the tool result. Do NOT call ask_human again "
            "and do not repeat the audit. Write ONLY the final Pass 2 Markmap "
            "markdown for the full transcript: use the resolved fact the "
            "student chose, record the resolution inline as 'Resolved per "
            "student: ...', keep the verify-later section, structured nodes only."
        )
